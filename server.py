...
# Reject images that are too small to survive full-screen scaling without pixelating.
# BBC upgraded images are normally 1024x576, so these limits keep good BBC photos
# but reject thumbnails / low-res fallback images from other sources.
MIN_IMAGE_LONG_SIDE = 900
MIN_IMAGE_SHORT_SIDE = 480
...
def image_meets_resolution_requirements(img):
    ...
    return long_side >= MIN_IMAGE_LONG_SIDE and short_side >= MIN_IMAGE_SHORT_SIDE
...
def fetch_text(url, timeout=2.5):
...
def extract_image_from_html_page(url):
    """Small capped fallback for RSS feeds that omit image tags."""
    try:
...
def article_link_from_item(item):
    link = item.find("link")
...
def get_bbc_images(limit=MAX_IMAGE_POOL):
    ...
    # Prevent Render/page-load hangs. BBC RSS is cheap; non-BBC article scraping is capped.
    start_time = time.time()
    time_budget_seconds = 7.5
    non_bbc_article_scrape_budget = 14
    ...
    for feed_url in feeds:
        ...
        try:
            ...
            item_limit = 650 if is_bbc_feed_url(feed_url) else 45
            ...
            for item in items[:item_limit]:
                ...
                if not img and not is_bbc_feed_url(feed_url):
                    if non_bbc_article_scrapes >= non_bbc_article_scrape_budget:
                        continue
                    link_url = article_link_from_item(item)
                    if link_url:
                        non_bbc_article_scrapes += 1
                        img = extract_image_from_html_page(link_url)
...
def crop_top_if_needed(img, url=""):
    ...
    try:
        ...
        # Small VOICE logo: crop only a little, not the larger BBC-branding crop.
...
def image_is_generic_single_portrait(data):
    try:
        arr = np.frombuffer(data, np.uint8)
        img = cv2.imdecode(arr, cv2.IMREAD_COLOR)
    except Exception:
    ...
    right = gray[:, int(gray.shape[1] * 0.8):]
    center = gray[:, int(gray.shape[1] * 0.3):int(gray.shape[1] * 0.7)]

    side_std = (float(np.std(left)) + float(np.std(right))) / 2.0
    center_std = float(np.std(center))

    # single-subject / isolated portrait feeling
    if center_std > side_std * 1.55 and edge_density < 0.095:
        return True

    # overly clean editorial portrait
    if side_std < 26 and center_std > 42:
        return True
        ...
        if path == "/proxy":
            ...
            try:
                ...
                try:
                    ...
                    if img is not None:
                        if not image_meets_resolution_requirements(img):
                            REJECT_CACHE[url] = {"time": time.time()}
                            h, w = img.shape[:2]
                            print(f"[REJECT low resolution {w}x{h}]", url)
                ...
                if image_is_generic_single_portrait(test_data):
                    REJECT_CACHE[url] = {"time": time.time()}
                    print("[REJECT portrait]", url)
                    self.safe_send_bytes(415, b"Rejected generic portrait", extra_headers={"Cache-Control": "no-store"})
                    return
