...
def main():
    ...
    failures += check(
        "sports/religion/photo-gallery pages are removed from direct/source pages",
        not any(any(term in page for term in ["sports", "religion", "photo-gallery", "ap-photos"]) for page in DIRECT_IMAGE_PAGES + SOURCE_PAGES),
    )
    failures += check(
        "politics/world article links are allowed",
        link_is_relevant_news_scene("https://apnews.com/article/politics-election-congress-example"),
    )
    failures += check(
        "entertainment/sports article links are rejected",
        not link_is_relevant_news_scene("https://apnews.com/article/entertainment-movies-celebrity-example")
        and not link_is_relevant_news_scene("https://apnews.com/article/sports-baseball-example"),
    )
